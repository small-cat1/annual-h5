<template>
  <div class="checkin-page">
    ...
    <div v-if="userStore.isCheckedIn" class="checked-box">
      ...
      <h2>签到成功</h2>
      <p class="time">签到时间：{{ formatDate(userStore.checkInInfo?.checkInTime) }}</p>
      
      <van-cell-group inset class="info-group">
        <van-cell title="姓名" :value="userStore.checkInInfo?.realName" />
        <van-cell title="手机号" :value="userStore.checkInInfo?.phone" />
        <van-cell title="部门" :value="userStore.checkInInfo?.department || '-'" />
        <van-cell title="工号" :value="userStore.checkInInfo?.employeeNo || '-'" />
      </van-cell-group>
